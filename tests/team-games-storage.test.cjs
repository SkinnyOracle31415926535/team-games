...
function loadStorage(initial = {}, { locks: useLocks = true } = {}) {
...
test('local Team Games saves stay available without browser locks while remote application defers', async () => {
  const environment = loadStorage({}, { locks: false });
  const next = environment.realm(appState({ selectedClassKey: 'builtin:level-3-boys' }));

  await environment.api.saveState(next);
  assert.equal(
    JSON.parse(environment.localStorage.getItem(keys.state)).selectedClassKey,
    'builtin:level-3-boys',
  );
  await assert.rejects(
    environment.api.makeAdapters().preferences.applyRemote(
      environment.realm({ version: 1, selectedClassKey: 'builtin:advanced-boys' }),
      remoteMetadata(false),
    ),
    /deferred until browser locking is available/,
  );
});
