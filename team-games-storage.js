(() => {
  ...
  const sha256 = async (value) => {
    if (!window.crypto || !window.crypto.subtle) {
      throw new Error('Secure hashing is required to automatically synchronize Team Games records.');
    }
  ...
  const withRemoteWrite = async (task) => {
    const locks = window.navigator && window.navigator.locks;
    if (!locks || typeof locks.request !== 'function') {
      throw new Error('Automatic Team Games updates are deferred until browser locking is available.');
    }
  ...
  const requireWriteSource = (metadata) => {
    if (!metadata || !['local', 'remote-migration'].includes(metadata.source)) {
      throw new Error('The automatic sync client requested an invalid local write source.');
    }
  };
